...
module.exports = {
    ...
    .setName("pepe-sign")
    .setDescription("Crea un emoji pepe sign")
    .addStringOption(option => option.setName("testo").setDescription("Il testo da mettere nel cartello").setRequired(true)),
    async execute(interaction, client) {
        ...
        const signText = interaction.options.getString("testo").trim();
        ...
        const embed = new EmbedBuilder()
            .setColor(config.embedFun)
            .setTitle(`${client.user.username} Pepe Sign ${config.arrowEmoji}`)
            .setImage("attachment://pepesign.png")
            .setFooter({ text: `Pepe sign created ${config.devBy}`})
            .setTimestamp();

        interaction.reply({ content: `<@${interaction.user.id}>, ecco il tuo cartello ${config.pepeCoffeeEmoji}`, embeds: [embed], files: [attachment] });
    }
}
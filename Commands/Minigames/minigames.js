...
const { TwoZeroFourEight, Minesweeper, Snake, Hangman, TicTacToe, Wordle, Connect4, GuessThePokemon, Slots, FindEmoji } = require('discord-gamecord');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('minigame')
        .setDescription('Gioca col bot.')
        .addSubcommand(command => command.setName('8ball').setDescription('Chiedi alla palla magica qualcosa.').addStringOption(option => option.setName('domanda').setDescription('Scrivi la domanda di cui vuoi ricevere una risposta.').setRequired(true).setMaxLength(200)))
        .addSubcommand(command => command.setName('2048').setDescription('Inizia una partita a 2048.'))
        .addSubcommand(command => command.setName('slots').setDescription('Inizia una partita alle slot.'))
        .addSubcommand(command => command.setName('cercaemoji').setDescription('Cerca l\'emoji nascosta. Riuscirai a trovarla?'))
        .addSubcommand(command => command.setName('campominato').setDescription('Inizia una partita a campo minato.'))
        .addSubcommand(command => command.setName('snake').setDescription('Inizia una partita a snake.'))
        .addSubcommand(command => command.setName('impiccato').setDescription('Inizia una partita di impiccato.'))
        .addSubcommand(command => command.setName('tris').setDescription('Inizia una partita di tris.').addUserOption(option => option.setName('avversario').setDescription(`Specifica l'avversario contro cui vuoi giocare.`).setRequired(true)))
        .addSubcommand(command => command.setName('wordle').setDescription('Inizia una partita di wordle.'))
        .addSubcommand(command => command.setName('preferiresti').setDescription('Hai 2 opzioni, quale sceglierai?'))
        .addSubcommand(command => command.setName('forza4').setDescription('Gioca a Forza Quattro.').addUserOption(option => option.setName('avversario').setDescription("Specifica l'avversario contro cui vuoi giocare.").setRequired(true)))
        .addSubcommand(command => command.setName('indovina-il-pokémon').setDescription('Gioca a Indovina il Pokémon, riuscirai a indovinarlo?')),
    ...
    async execute(interaction) {
        ...
        switch (sub) {
            case '8ball':
                ...
                const choice = ["È certo.", "È decisamente così.", "Senza nessun dubbio.", "Sì sicuramente.", "Si può fare affidamento su di esso.", "Per come la vedo io, sì.", "Molto probabile.", "Prospettive buone.", "Si.", "I segni indicano sì.", "Domanda confusa, riprova ancora.", "Chiedi di nuovo più tardi.", "Meglio non dirtelo ora.", "Non posso prevedere ora.", "Concentrati e chiedilo di nuovo.", "Non contare su di esso.", "La mia risposta è no.", "Le mie fonti dicono no.", "Le prospettive non sono buone.", "Molto dubbio.", "Lorenzo dice di no, sono d'accordo con lui :(", "Lorenzo dice di si, sono d'accordo con lui :)"]
                const ball = Math.floor(Math.random() * choice.length)

                try {
                    const embed = new EmbedBuilder()
                        .setColor("#e1c4ff")
                        .setAuthor({ name: '🎱 8ball ha la risposta' })
                        .setTitle(`> La domanda "${question}" \n> è stata ricevuta`)
                        .addFields({ name: `• Gira la palla`, value: `> Gira la palla magica per scoprire cos'ha da dire.` })

                    const embed2 = new EmbedBuilder()
                        .setColor("#e1c4ff")
                        .setAuthor({ name: '🎱 8ball ha la risposta' })
                        .setTitle(`> La domanda "${question}" è stata giudicata dalla palla magica.`)
                        .addFields({ name: `• Risposta`, value: `> ${choice[ball]}` })

                    const button = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                .setCustomId('button')
                                .setLabel('🎱 Gira la palla')
            ...
            case '2048':
                try {
                    const Game = new TwoZeroFourEight({
                        ...
                        timeoutTime: 60000,
                        buttonStyle: 'SUCCESS',
                        playerOnlyMessage: 'Non puoi usare questo menù!'
                    });
                ...
                try {
                    const Game = new FindEmoji({
                        ...
                        embed: {
                          title: 'Trova l\'emoji',
                          color: '#e1c4ff',
                          description: 'Ricorda le emoji qui sotto.',
                          findDescription: 'Trova l\'emoji {emoji} prima che finisca il tempo.'
                        },
                        timeoutTime: 60000,
                        hideEmojiTime: 5000,
                        buttonStyle: 'PRIMARY',
                        emojis: ['🍉', '🍇', '🍊', '🍋', '🥭', '🍎', '🍏', '🥝'],
                        winMessage: 'Hai vinto! Hai selezionato l\'emoji corretta. {emoji}',
                        loseMessage: 'Hai perso! Hai selezionato l\'emoji sbagliata. {emoji}',
                        timeoutMessage: 'Hai perso! Hai finito il tempo a tua disposizione. L\'emoji era {emoji}',
                        playerOnlyMessage: 'Solo {player} può usare questi bottoni.'
                      });
                ...
                try {
                    const Game1 = new Minesweeper({
                        ...
                        embed: {
                            title: 'Minesweeper',
                            color: '#e1c4ff',
                            description: 'Fare clic sui pulsanti per rivelare i blocchi.'
                        },
                        emojis: { flag: '🚩', mine: '💣' },
                        mines: 5,
                        timeoutTime: 60000,
                        winMessage: 'Hai vinto il gioco! Hai evitato con successo tutte le mine.',
                        loseMessage: 'Hai perso il gioco! Fai più attenzione alle mine la prossima volta :(',
                        playerOnlyMessage: 'Non puoi usare questo menù!'
                    });
            ...
            case 'impiccato':
                try {
                    const Game3 = new Hangman({
                        message: interaction,
                        embed: {
                            title: '> Hai iniziato una partita di impiccato!',
                            color: '#e1c4ff'
                        },
                        hangman: { hat: "🎩", head: "😟", shirt: "👕", pants: "👖", boots: "🥾🥾" },
                        timeoutTime: 60000,
                        themeWords: "all",
                        winMessage: "> Hai vinto! La parola era **{word}**",
                        loseMessage: "> Hai perso! La parola era **{word}**",
                        playerOnlyMessage: "Solo **{player}** può usare questo menù!",
                    })
                ...
                break;
            case 'tris':
                const enemy = interaction.options.getUser('avversario');
                if (interaction.user.id === enemy.id) return await interaction.reply({ content: `Non puoi giocare contro te stesso.` });
                if (enemy.bot) return await interaction.reply({ content: `Non puoi giocare contro un bot.` });

                try {
                    const game = new TicTacToe({
                        message: interaction,
                        isSlashGame: true,
                        opponent: interaction.options.getUser('avversario'),
                        ...
                        turnMessage: '> **{player}**, è il tuo turno!.',
                        winMessage: '> **{player}** hai vinto!',
                        tieMessage: '> La partita si è conclusa in pareggio!',
                        timeoutMessage: '> La partita è finita! Nessuno ha vinto la partita!',
                        playerOnlyMessage: 'Solo {player} e {opponent} possono usare questi bottoni.'
                    });

                    try {
                        await game.startGame();
                    } catch (err) {
                        await interaction.reply("Errore durante l'avvio della partita!");
                    }
            ...
            case 'wordle':
                try {
                    const Game4 = new Wordle({
                        ...
                        isSlashGame: false,
                        embed: {
                            title: '> Hai iniziato una partita a Wordle! \n> Scrivi una parola in chat e prova ad indovinare.',
                            color: '#e1c4ff'
                        },
                        customword: null,
                        timeoutTime: 120000,
            ...
            case 'preferiresti':
                ...
                try {
                    const embed3 = new EmbedBuilder()
                        .setTitle('• Preferiresti')
                        .setColor('#e1c4ff')
                        .setDescription(`> ${randomize}`)
                        .setAuthor({ name: `🤔 Preferiresti` })

                    const embed1 = new EmbedBuilder()
                        .setTitle('Preferiresti')
                        .setColor('#e1c4ff')
                        .setDescription('> Hai scelto **opzione 1**')
                        .setAuthor({ name: `🤔 Preferiresti` })
                    ...
                    const embed5 = new EmbedBuilder()
                        ...
                        .setAuthor({ name: `🤔 Preferiresti` })
                        .setColor('#e1c4ff')
                        .setDescription('> Hai scelto **opzione 2**')
                    ...
                    collector1.on('collect', async i => {
                        if (i.customId === 'option1') {
                            if (i.user.id !== interaction.user.id) {
                                return await i.reply({ content: "Non puoi usare questo menù.", ephemeral: true })
                            }
                ...
                try {
                    ...
                    const game = new Connect4({
                        ...
                        embed: {
                            ...
                            rejectTitle: "Richiesta Cancellata",
                            statusTitle: "• Stato",
                            overTitle: "• Partita Finita",
                            color: "#e1c4ff",
                            rejectColor: "#FF0000",
                        ...
                        timeoutTime: 120000,
                        buttonStyle: "PRIMARY",
                        turnMessage: "> {emoji} | **{player}**, è il tuo turno!",
                        winMessage: "> 🎉 | **{player}** ha vinto la sfida a Forza4!",
                        tieMessage: "> La sfida è finita in pareggio!",
                        timeoutMessage: "> La sfida non è stata finita, nessuno ha vinto!",
                        playerOnlyMessage: "Solo {player} e {opponent} possono usare questi bottoni.",
                        rejectMessage: "{opponent} ha rifiutato l'invito a giocare a Forza4!",
                    });
            ...
            case 'indovina-il-pokémon':
                try {
                    const game = new GuessThePokemon({
                        ...
                        timeoutTime: 60000,
                        winMessage: '> 🎉 | Hai indovinato! Era {pokemon}.',
                        loseMessage: '> Sarai più fortunatə la prossima volta! Era {pokemon}.',
                        errMessage: 'Impossibile trovare il Pokémon! Per favore riprova più tardi.',
                        timeoutMessage: '> La partita non è stata finita.',
                        playerOnlyMessage: 'Solo {player} può usare questi bottoni.'
                    });
